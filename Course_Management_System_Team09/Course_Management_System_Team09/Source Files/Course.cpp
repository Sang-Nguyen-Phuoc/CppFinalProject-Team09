...
void getmark(Course *course, student *sHead)
{
    ...
    while (getline(file, line))
    {
        ...
        while (run)
        {
            if (run->studentID == temp->studentID)
                break;
            run = run->next;
        }
...
void changecourseID(Course *course)
{
    ...
    void deleteonestudent(Course * course)
    {
        ...
        i = 1;
        student *temp = cur->liststudent;
        cout << left << setw(4) << "No" << setw(15) << "Student ID" << setw(20) << "First Name"
             << "Last Name" << endl;
        while (temp)
        {
    ...
    void updateresult(Course * course, student * sHead)
    {
        ...
        while (run2)
        {
            if (run2->courseName == cur->courseName)
                break;
            run2 = run2->next;
        }
